...
def make_cert(request):
    ...
    if request.method=="POST":
        ...
        if len(user)==0:
            ...
            return None
        the_user = user[0]
        Certificate(userc=the_user, cert_type=cert, note=request.POST.get("note"), date=date.today()).save()
...
def history(request):
    if request.method=="GET":
        ...
        form = request.POST
        name_nik = form.get("name_nik")
        users = list(UserC.objects.filter(Q(name__iregex=rf".*{name_nik}.*")|Q(nik__iregex=rf".*{name_nik}.*"))) 
        certs = []
        for u in users:
            certs += (list(Certificate.objects.filter(userc=u)))
